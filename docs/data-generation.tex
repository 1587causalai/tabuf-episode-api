...
$M$ 是 World 级缺失（格子在世界上就没了）。
$Q$ 是 Compiler 的 value mask（考题）。
两者都按全表比例抽，互不相交。网络看见的 \texttt{values} 只在 $C$ 上非空。
...
\texttt{query\_frac} & 0.15 & value mask，占全表比例 \\
\texttt{missing\_frac} & 0 & 世界缺失，占全表比例；JSON 始终带 mask \\
