...
\texttt{openml} & 占位 & 分类/回归；默认 query 整列标签 \\
\texttt{recsys} & 占位 & User$\times$Item；默认 query 已观测评分的子集 \\
\bottomrule
\end{tabular}
\end{center}

\texttt{GET /v0/sources} 返回每条来源的画像（status、task、默认 query\_mode / missing\_frac）。分类回归（sklearn\_real / sklearn\_synthetic / openml）默认 \texttt{query\_mode=label\_column}：最后一列是标签，整列进 $Q$。DiscoSCM 与 SCM 仍是格子级。请求里的 missing/query/query\_mode 为 null 时用画像默认。
