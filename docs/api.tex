...
\texttt{return\_mechanism} & bool & false & --- & 是否返回该集 DiscoSCM \\
\texttt{debug} & bool & false & --- & 额外满表 $Y_{\mathrm{full}}$；隐含上一开关 \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{\texttt{return\_mechanism}：每集背后的 DiscoSCM}

默认\textbf{不}返回生成机制。训练路径必须保持关闭。
打开后，每个 episode 多一个 \texttt{mechanism} 对象，按 DiscoSCM 元组 $\langle U,\mathbf{E},\mathbf{V},\mathcal{F}\rangle$ 给出这一集的世界是怎么来的。
...
$U$ 是 unit selection，不是 SCM 的外生噪声。第零版 $\mathbf{E}$ 是一次观测噪声，不重抽 $\mathbf{E}(\mathbf{x})$。
用 $U,W,E$ 可以还原满表 $Y=UW^{\top}+E$，因此机制通道\textbf{不}再附带 $Y_{\mathrm{full}}$。
\texttt{debug=true} 才会额外给出顶层 $U,W,Y_{\mathrm{full}}$（兼容旧字段），并隐含 \texttt{return\_mechanism=true}。

\begin{warnbox}
\texttt{return\_mechanism} 与 \texttt{debug} 都是漏题开关。DataLoader 必须保持 false。
需要机制的场景：检查生成器、写识别实验、对拍 Compiler。不要把 \texttt{mechanism} 喂进网络。
\end{warnbox}
