...
一次返回 $n_{\mathrm{ep}}$ 条 episode。每条是\textbf{一张带 mask 的表}，不是一行。
生成顺序：抽总体 $\{\mathbf{u}_i\}$ $\to$ 用 unit-specific law 填格 $\to$ 世界缺失 $\to$ 再在剩余格子上出查询题。
...
\texttt{query\_frac} & float & 0.15 & $(0,1)$ & value mask，占全表比例 \\
\texttt{missing\_frac} & float & 0 & $[0,0.9)$ & 世界缺失，占全表比例 \\
\texttt{sigma} & float & 0.3 & $[0,10]$ & 观测噪声标准差 \\
\texttt{seed} & int/null & 0 & --- & 基种子；第 $e$ 集用 $\mathrm{seed}+e$ \\
\texttt{n\_episodes} & int & 1 & $[1,32]$ & 一次返回多少条 episode \\
\texttt{return\_mechanism} & bool & false & --- & 是否带总体和响应律 \\
\texttt{debug} & bool & false & --- & 额外 $Y_{\mathrm{full}}$；隐含上一开关 \\
\bottomrule
\end{tabular}
\end{center}

\texttt{query\_frac} 与 \texttt{missing\_frac} 都按全表格子计，抽成互不相交的两套。
至少留 1 个上下文格和 1 个查询格。JSON 里\textbf{永远}有 \texttt{missing\_mask}，即使默认全 false。
  ...
  "episodes": [
    {
      ...
      "table": {
        ...
        "query_mask":   [[false, false, ...], ...],
        "y_query": [0.87, ...],
        "shapes": {"n_missing": 6, "n_context": 48, "n_query": 10, ...}
      }
    }
  ]
}
...
默认每条 episode 只有 \texttt{seed} 和 \texttt{table}。
...
字段 & 形状 & 含义 \\
\midrule
\texttt{table.values} & $n\times d$ & 仅 $C$ 有数；缺失和查询为 \texttt{null} \\
\texttt{table.missing\_mask} & $n\times d$ bool & World 缺失 $M$ \\
\texttt{table.context\_mask} & $n\times d$ bool & 网络可见 $C$ \\
\texttt{table.query\_mask} & $n\times d$ bool & 考题 $Q$（value mask） \\
\texttt{table.y\_query} & $n_Q$ & $Q$ 上真值，行优先 \\
\texttt{population} & 对象 & 仅 \texttt{return\_mechanism} \\
\texttt{response\_law} & 对象 & 仅 \texttt{return\_mechanism} \\
\texttt{Y\_full} & $n\times d$ & 仅 \texttt{debug} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{align*}
  M \cap C = C \cap Q = M \cap Q &= \emptyset,\\
  M \cup C \cup Q &= \{1,\ldots,n\}\times\{1,\ldots,d\}.
\end{align*}

\texttt{values} 非空当且仅当该格 $\in C$。

\begin{warnbox}
训练 DataLoader 只吃 \texttt{table}。禁止 \texttt{return\_mechanism} / \texttt{debug}，禁止把 $u_i$ 喂进网络。
\end{warnbox}
...
第零版 $E$ 只抽一次，不重抽反事实噪声。
用表征、$W$、$E$ 可还原满表。\texttt{debug} 才会再给 \texttt{Y\_full}。
