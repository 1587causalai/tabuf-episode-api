...
列类型按权重 $70{:}10{:}10{:}5$ 直接抽（数值 / 有序 / 多值类别 / 超多值；和为 $95$，归一化后多项抽样）。每列再以 $0.05$ 的概率与其它特征独立。生成：数值 $Y=\mathrm{scale}\langle u,w\rangle+\mathrm{shift}+e$；有序为 ordered probit；多值与超多值为 unit-specific multinomial logit（Gumbel 噪声），超多值类嵌入尺度更散、频率更偏。
